...
class Program:
    def __init__(self, name, downloaded_versions, parametrs=None):
        ...
        if parametrs:
            ...
            self.download_link = parametrs.setdefault(
                'download_link', self.url)
        ...
        self.downloaded_version = downloaded_versions[self.name.replace(
            "'", '')] if self.name.replace("'", '') in downloaded_versions.keys() else 'Not Found'

        self.have_update = all(
            (self.downloaded_version != self.current_version, not self.error))

    def current_version_check(self):            
        try:
            session = get(self.url, timeout=5)
        except:
            self.error = 'No response to the request'
        else:
            html = BeautifulSoup(session.text, 'html.parser')
            ...
            if all([self.content, element_response]):
                ...
                if isinstance(self.word, bool):
                    ...
                    self.current_version = ' '.join(
                        text[self.slice_start:self.slice_end])
            else:
                self.error = f'Incorrect response or parametrs: {self.content, element_response}'

        if self.error and not self.current_version:
            self.current_version = 0
